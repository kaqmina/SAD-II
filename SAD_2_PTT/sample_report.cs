...
namespace SAD_2_PTT
{
    public partial class sample_report : Form
    {
        ...
        public sample_report()
        {
            ...
            rep.project_grid(project_grid);
            project_grid.ClearSelection();

            date_from.Visible = date_to.Visible =pnl_device.Visible = pnl_project.Visible =  false;
        ...
        {
            if (dev_status.SelectedIndex == 0) rep.device_status_grid(device_grid, "default", from, to, end, format);
            else if (dev_status.SelectedIndex == 1) rep.device_status_grid(device_grid, "requested", from, to, end, format);
            else if (dev_status.SelectedIndex == 2) rep.device_status_grid(device_grid, "handed out", from, to, end, format);
        }

        #endregion

        #region << PROJECT >>
        public string project, place;
        public DateTime date_proposed;
        public string function = "Export PDF of Participants";
        private void project_grid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            DataGridViewRow row = this.project_grid.Rows[e.RowIndex];
            project = row.Cells["project_title"].Value.ToString();
            place = row.Cells["event_held"].Value.ToString();
            date_proposed = Convert.ToDateTime(row.Cells["date_proposed"].Value.ToString());

            rep.getProjectID(project);
        }

        private void button6_Click(object sender, EventArgs e) // list of participants
        {
            string file = "";
            save_pdf.Filter = "PDF files |*.pdf";
            save_pdf.DefaultExt = "*.pdf";
            ...
            {
                rep.project_attendance(file, project, date_proposed.ToString("MMMM dd, yyyy"), place);
                System.Diagnostics.Process.Start(file); // to open document directly after creating PDF
            }
            save_pdf.FileName = "";
        ...
        private void export_project_Click(object sender, EventArgs e)
        {
            string file = "";
            string date = "";
            save_pdf.Filter = "PDF files |*.pdf";
            ...
            {
                rep.project_reportPDF(file, date, project_grid);
                System.Diagnostics.Process.Start(file); // to open document directly after creating PDF
            }
            save_pdf.FileName = "";
        }
        ...
        #region Temporary [Button Panel]
        private void btn_pwd_Click(object sender, EventArgs e)
        {
            pnl_pwd.Visible = true;
            pnl_device.Visible = pnl_project.Visible = false;
            lbl_dist.Visible = district_format.Visible = true;
        }
        private void btn_device_Click(object sender, EventArgs e)
        {
            pnl_pwd.Visible = pnl_project.Visible = false;
            pnl_device.Visible = true;
            lbl_dist.Visible = district_format.Visible = false;

        }
        private void btn_project_Click(object sender, EventArgs e)
        {
            pnl_pwd.Visible = pnl_device.Visible = false;
