...
namespace SAD_2_PTT
{
    public partial class device_provider : Form
    {
        ...
        String dp_name, dp_desc, tel_no, email, search;
        int dp_id, mob_no, dp_type;
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else
            {
                ...
                dp_type = Convert.ToInt32(row.Cells["dp_type"].Value.ToString());

                dp_id = Convert.ToInt32(row.Cells["dp_id"].Value);
                mob_no = Convert.ToInt32(row.Cells["mobile_no"].Value);

                //dp_type
                if (dp_type == 0) cmbox_type.Text = "Government";
                else if (dp_type == 1) cmbox_type.Text = "Sponsor";
